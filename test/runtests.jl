...
@testset "Augmented ODEProblems" begin
    ...
    @testset "Cubic OOP" begin
        ...
        @test augsol.u[end].lfwd≈first(quadgk(t -> M(f(solfwd(t), nothing, t), nothing,
                                                     nothing, nothing), 0.0, 5.0)) atol=0.01
        @test augsol.u[end].lbwd≈first(quadgk(t -> M(f(solbwd(t), nothing, t), nothing,
                                                     nothing, nothing), 0.0, 5.0)) atol=0.01
    end
    ...
    @testset "Cubic IIP" begin
        ...
        @test augsol.u[end].lfwd≈first(quadgk(t -> M(f.(solfwd(t)), nothing, nothing,
                                                     nothing), 0.0, 5.0)) rtol=0.01
        @test augsol.u[end].lbwd≈first(quadgk(t -> M(f.(solbwd(t)), nothing, nothing,
                                                     nothing), 0.0, 5.0)) atol=0.01
    end
end
